...
def _setup_arrival_calcs(
    ...
    **kwargs: Union[str, int, float]
    ...
    """ Helper method which sets up the data needed for the arrival rate calculations.
    This is a separate cached method as these data are not effected by the traffic
    (spout_state) and so do not need to be recalculated for a new traffic level for the
    same topology id/ref. """
    ...
    i2i_rps: pd.Series = (
        calculate_inter_instance_rps(
            ...
            tracker_url,
            **kwargs
        ).set_index(["source_task", "destination_task", "stream"])[
            "routing_probability"
        ]
    )
    ...
    LOG.info(
        "Calculating levels for topology %s reference %s", topology_id, topology_ref
    )
    levels: List[List[Vertex]] = get_levels(topo_traversal)
    LOG.debug(
        "Found %d levels is topology %s reference %s",
        len(levels),
        topology_id,
        topology_ref,
    )
    ...
    coefficients: pd.Series = lstsq_io_ratios(
        ...
        io_bucket_length,
        **kwargs
    ).set_index(["task", "output_stream", "input_stream", "source_component"])[
        "coefficient"
    ]
    ...
    receiving_instances: Dict[Vertex, List[int]] = (
        ...
        .group()
        .by("id")
        .by(out("physically_connected").hasLabel("bolt").values("task_id").fold())
        .next()
    )

    return (i2i_rps, levels, coefficients, sending_instances, receiving_instances)
...
def _calculate_arrivals(
    ...
    LOG.debug(
        "Processing output from source instance %s_%d", source_component, source_task
    )

    for out_edge in out_edges:
        stream: str = cast(str, out_edge["stream_name"])
        try:
            stream_output: float = cast(float, output_rates[source_task][stream])
        except KeyError:
            LOG.debug(
                "No output rate information for source task %d on "
                "stream %s. Skipping the outgoing edge",
                source_task,
                stream,
            )
            continue
        ...
        try:
            ...
            LOG.debug(
                ...
                source_component,
                source_task,
                out_edge["destination_component"],
                destination_task,
                stream,
                stream_output,
                r_prob,
        ...
        arrival_rates[destination_task][(stream, source_component)] += edge_output
...
def _calculate_outputs(
    ...
    for out_stream in out_streams:
        ...
        for in_stream in in_streams:
            ...
            stream_arrivals: float = arrival_rates[source_task][
                (in_stream_name, source_component)
            ]

            try:
                coefficent: float = float(
                    coefficients.loc[
...
def _convert_arrs_to_df(arrival_rates: ARRIVAL_RATES) -> pd.DataFrame:
    ...
    for task_id, incoming_streams_dict in arrival_rates.items():
        for (
            (incoming_stream, source_component),
            arrival_rate,
...
def _calc_strmgr_in_out(
    sending_instances: Dict[str, List[int]],
    ...
    **kwargs: Union[str, int, float]
    ...
    for level_number, level in enumerate(levels[:-1]):
        ...
        for source_vertex in level:
            ...
            arrival_rates = _calculate_arrivals(
                topo_traversal, source_vertex, arrival_rates, output_rates, i2i_rps
            )
