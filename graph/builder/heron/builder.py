...
def _create_stream_managers(
    ...
    for stream_manager in physical_plan["stmgrs"].values():
        ...
        LOG.debug("Creating vertex for stream manager: %s", stream_manager["id"])

        strmg: Vertex = (
            graph_client.graph_traversal.addV("stream_manager")
        ...
        cont: Vertex = (
            graph_client.graph_traversal.addV("container")
        ...
        LOG.debug(
            "Connecting stream manager %s to be within container %d",
            stream_manager["id"],
            container,
        )
        (graph_client.graph_traversal.V(strmg).addE("is_within").to(cont).next())
...
def _create_spouts(
    ...
    for spout_name, spout_data in logical_plan["spouts"].items():
        LOG.debug("Creating vertices for instances of spout component: %s", spout_name)
        for instance_name in physical_spouts[spout_name]:

            instance: Dict[str, Union[str, int]] = tracker.parse_instance_name(
                instance_name
            )

            LOG.debug("Creating vertex for spout instance: %s", instance_name)

            stream_manager_id: str = physical_plan["instances"][instance_name][
                "stmgrId"
            ]

            spout: Vertex = (
                graph_client.graph_traversal.addV("spout")
            ...
            (
                graph_client.graph_traversal.V(spout)
                .addE("is_within")
                .to(
...
def _create_bolts(
    ...
    for bolt_name in logical_plan["bolts"]:
        LOG.debug("Creating vertices for instances of bolt component: %s", bolt_name)
        for instance_name in physical_bolts[bolt_name]:
            ...
            LOG.debug("Creating vertex for bolt instance: %s", instance_name)
            ...
            bolt: Vertex = (
                graph_client.graph_traversal.addV("bolt")
                .property("container", instance["container"])
                .property("task_id", instance["task_id"])
            ...
            (
                graph_client.graph_traversal.V(bolt)
                .addE("is_within")
                .to(
                    graph_client.graph_traversal.V()
                    .hasLabel("container")
...
def _create_logical_connections(
    graph_client: GremlinClient,
    topology_id: str,
    topology_ref: str,
    logical_plan: Dict[str, Any],
    ...
    LOG.info("Adding logical connections to topology %s instances", topology_id)
    ...
    for bolt_name, bolt_data in logical_plan["bolts"].items():
        ...
        LOG.debug(
            "Adding logical connections for instances of " "destination bolt: %s",
            bolt_name,
        )

        # Get a list of all instance vertices for this bolt
        destination_instances: List[Vertex] = (
            topo_traversal.V().has("component", bolt_name).toList()
        )
            ...
            for destination in destination_instances:
                for source in source_instances:
                    (
                        ...
                        .property("stream", incoming_stream["stream_name"])
...
def _create_physical_connections(
    ...
    LOG.info(
        "Creating physical connections for topology: %s, reference: " "%s",
        topology_id,
        topology_ref,
    )
    ...
    logical_edges: List[Dict[str, Union[Vertex, Edge]]] = (
        topo_traversal.V()
        .hasLabel(P.within("bolt", "spout"))
        .outE("logically_connected")
        .project(
            "source_instance",
            "source_container",
            "source_stream_manager",
            "l_edge",
            "destination_instance",
            "destination_container",
            "destination_stream_manager",
        )
        .by(outV())
    ...
    for logical_edge in logical_edges:
        ...
        destination_stream_manager: Vertex = logical_edge["destination_stream_manager"]
        ...
        if source_container == destination_container:
            ...
            graph_client.graph_traversal.E(l_edge).property("type", "local").next()
            ...
            graph_client.graph_traversal.E(l_edge).property("type", "remote").next()


def create_physical_graph(
    graph_client: GremlinClient,
    topology_id: str,
    topology_ref: str,
    logical_plan: Dict[str, Any],
    ...
    if graph_client.topology_ref_exists(topology_id, topology_ref):
        msg: str = (
            f"A graph of topology {topology_id} with reference "
    ...
    LOG.info(
        "Building physical graph for topology %s with reference %s",
        topology_id,
        topology_ref,
    )
    ...
    _create_stream_managers(graph_client, topology_id, topology_ref, physical_plan)

    _create_spouts(graph_client, topology_id, topology_ref, physical_plan, logical_plan)

    _create_bolts(graph_client, topology_id, topology_ref, physical_plan, logical_plan)

    _create_logical_connections(graph_client, topology_id, topology_ref, logical_plan)

    _create_physical_connections(graph_client, topology_id, topology_ref)

    LOG.info(
        "Physical graph construction completed after %d seconds",
        (dt.datetime.now() - start).total_seconds(),
    )


def populate_physical_graph(
    graph_client: GremlinClient,
    ...
    LOG.info(
        "Populating topology %s reference %s physical graph with "
    ...
    set_fields_routing_probs(
        graph_client, metrics_client, topology_id, topology_ref, start, end
    )
